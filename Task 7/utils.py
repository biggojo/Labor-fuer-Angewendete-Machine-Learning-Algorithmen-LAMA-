...
def get_output(model, layer_name, model_input):
    ...
    # The keras versions served with TF1 and TF2 have different behaviour, so we need to check for them and use the right method
    if tf.__version__.startswith('1'):
        return partial_model([model_input.astype('float32')]).eval(session=K.get_session())
    else:
...
def augment_images(images, h_flip=True, rotate180=True, shift_random=True):
    ...
    if h_flip:
        ...
        images_h_flip = np.zeros(images.shape, dtype='uint8')
    ...
    if rotate180:
        ...
        images_rot180 = np.zeros(images.shape, dtype='uint8')
    ...
    if shift_random:
        ...
        shift_images = np.zeros(images.shape, dtype='uint8')
